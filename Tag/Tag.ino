...
// Define the default destination ID
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (d1 == 'd') {  // Set destination ID
      DESTINATION_ID = data.substring(2).toInt();
      Serial.print("Destination ID set to: ");
...
void sendMessage() {
  if (strlen(uMessage) > 0) {    // Only proceed if the message is not empty
    ...
    SEQ++;
    snprintf(str, sizeof(str), "%5d %5d %5d %5d %5d %5d %5d %5d %s", SEQ, TYPE, TAGID, RELAYID, TTL, thisRSSI, SENDER_ID, DESTINATION_ID, uMessage);
